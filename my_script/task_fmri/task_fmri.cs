...
public class task_fmri : MonoBehaviour
{
    ...
    bool noITI = true;
    bool fMRI_key = false;
    bool eyelink_involved = false;

    bool Is1stTrial = false;
    ...
    Stopwatch trial_time = new Stopwatch();
    ...
    void ShowGUI(int windowID)
    {
        ...
        if (GUI.Button(new Rect((250 - 100) / 2, 200, 100, 30), "OK"))
        {
            ...
            if (eyelink_involved)
            {
                eyelink_fileName = "el_" + subject_number + ".edf";
                eyelink = new elconnect.ELmain();
            }
            task_fmri_create_trialTable_person create_trial_table = wooden_fence.GetComponent<task_fmri_create_trialTable_person>();
    ...
    void noise_screen1()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("noise_after_iti");
        }
        if (noITI)
        {
    ...
    void Update()
    {
        ...
        if (Is1stTrial)
        {
            if (TRstartCounting & Math.Round(trial_time.ElapsedMilliseconds * 0.001f, 2) % 2 == 0)
            {
                Is1stTrial = false;
                trial_time.Reset();
                TRstartCounting = false;
                Invoke("noise_screen1", 0);
            }
        }
        else if (!Is1stTrial & init_finish)
        {
            if (reorganized_DataSet[trial_order.ElementAt(trial_ith)][4] == "0")
            {
                if (TRstartCounting & (Math.Round((trial_time.ElapsedMilliseconds + ReactionTime) * 0.001f, 2) + 2.5f) % 2 == 0)
                {
                    trial_time.Reset();
                    TRstartCounting = false;
                    Invoke("noise_screen1", 0);
            ...
            {
                if (TRstartCounting & Math.Round((trial_time.ElapsedMilliseconds + ReactionTime) * 0.001f, 2) % 2 == 0)
                {
                    trial_time.Reset();
                    TRstartCounting = false;
                    Invoke("noise_screen1", 0);
        ...
        if (isOn_calibration_screen & Input.GetKeyDown(KeyCode.E))
        {
            isOn_calibration_screen = false;
            Screen.SetResolution(1024, 768, false, 60);
            ...
            if (eyelink_involved)
            {
                ...
                {
                    eyelink.open_elW();
                    eyelink.generate_el_file(eyelink_fileName);
                    eyelink.calib();
                }
                if (run_ith == 2)
                {
    ...
    void show_head_profile()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("idle head displayed");
        }
    ...
    void write_to_text()
    {
        ...
        //UnityEngine.Debug.Log(trial_ith);
    ...
    void noise_after_walking()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("noise after walking");
        }
        if (noITI)
        {
    ...
    void show_facing_person()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("facing idle displayed");
        }
    ...
    void noise_after_facing()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("noise after facing");
        }
        if (noITI)
        {
    ...
    void fixation4()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("noise after goal");
        }
        if (noITI)
        {
            show_question();
    ...
    void show_question()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.add_label("cue displayed");
        }
    ...
    void thank_you()
    {
        ...
        if (eyelink_involved)
        {
            eyelink.stopRecord(eyelink_fileName);
        }
        showText("实验结束\n\n" + "你的被试费是 " + subject_preformance_fee.ToString() + " 元", 0, 50);
        exp_time.Stop();

    }
    void pre_calibration()
    {
        Destroy(canvasobject);
        showBackgroundScreen();
        showText("任务开始后请持续注视白点\n\n准备好后按‘1’键，有问题按‘4’键", 0, 50);
        isOn_calibration_screen = true;
    }
